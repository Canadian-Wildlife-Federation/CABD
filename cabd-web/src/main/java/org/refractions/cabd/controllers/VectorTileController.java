...
/**
 * Controller for providing barriers as vector tiles
 * 
 * @author Emily
 *
 */
@RestController
@RequestMapping("/" + VectorTileController.PATH)
public class VectorTileController {
	...
	private byte[] getTileInternal(int z, int x, int y, FeatureType ftype) {
		...
		if (tile == null || tile.length == 0) {
			tile = featureDao.getVectorTile(z, x, y, ftype); 
			if (tile == null || tile.length == 0)
				return new byte[0];
		}
	...
	private String getTileKey(int z, int x, int y, FeatureType layer) {
		...
		return ft + "_" + z + "_" + x + "_" + y + "_" + LocaleContextHolder.getLocale().getLanguage().toLowerCase();
	}
