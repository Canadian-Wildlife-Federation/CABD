...
if dataFile == '':
    print("Data file required.  Usage: cabd_dams_wrispublicdams.py <datafile>")
    sys.exit()
...
update {workingTable} set operating_status_code = 
    ...
    WHEN DAM_OPERATION_CODE = 'Application' THEN 5
    WHEN DAM_OPERATION_CODE = 'Breached (Failed)' THEN 5
    WHEN DAM_OPERATION_CODE = 'Deactivated' THEN 4
    WHEN DAM_OPERATION_CODE = 'Decommissioned' THEN 3
    WHEN DAM_OPERATION_CODE = 'Not Constructed' THEN 5
...
update {workingTable} set construction_year = cast(COMMISSIONED_YEAR as numeric);
...
update {workingTable} set construction_type_code =
    CASE 
    WHEN DAM_TYPE = 'Combination' THEN 9
    WHEN DAM_TYPE = 'Concrete' THEN 9
    ...
    WHEN DAM_TYPE = 'Embankment–homogenous' THEN 3
    WHEN DAM_TYPE = 'Embankment–unknown' THEN 3
    WHEN DAM_TYPE = 'Embankment–zoned' THEN 3
    WHEN DAM_TYPE = 'Log crib' THEN 8
    WHEN DAM_TYPE = 'Other' THEN 10
    WHEN DAM_TYPE = 'Rockfill' THEN 6
    WHEN DAM_TYPE = 'Steel' THEN 7
    WHEN DAM_TYPE = 'Unknown' THEN 9
    ELSE NULL end;
update {workingTable} set height_m = DAM_HEIGHT_IN_METRES;
update {workingTable} set length_m = CREST_LENGTH_IN_METRES;
...
--update {workingTable} set spillway_type_code = CASE WHERE SPILLWAY_TYPE = THEN;
...
update {workingTable} set data_source = 'WRIS_Public_Dams';
