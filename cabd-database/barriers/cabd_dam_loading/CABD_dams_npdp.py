...
if dataFile == '':
    print("Data file required.  Usage: cabd_dams_npdp.py <datafile>")
    sys.exit()
...
alter table {workingTable} rename latitude to latitude_orig;
alter table {workingTable} rename longitude to longitude_orig;
...
update {workingTable} set dam_name_en = 'dam name';
--update {workingTable} set dam_name_fr;
update {workingTable} set waterbody_name_en = River;
...
update {workingTable} set province_territory_code = 
    CASE 
    WHEN "state/ province" = 'ON/QC' THEN 'on'
    ...
    ELSE NULL END;
update {workingTable} set nearest_municipality = 'nearest downstream city';
...
update {workingTable} set construction_year = "year completed";
--update {workingTable} set assess_schedule;
--update {workingTable} set expected_life;
update {workingTable} set maintenance_last = cast('01-01-' || "year modified" as date);
...
update {workingTable} set construction_type_code = 
    ...
    WHEN "dam type" IS NULL THEN 9 END;
update {workingTable} set height_m = "dam height (m)";
update {workingTable} set length_m = "dam lengh (m)";
...
update {workingTable} set storage_capacity_mcm = cast(replace("normal reservoir storage (m3)", ',', '') as double precision);
...
update {workingTable} set generating_capacity_mwh = "electric capacity";
