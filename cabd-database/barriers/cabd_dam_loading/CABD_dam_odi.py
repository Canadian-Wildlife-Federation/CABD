...
if dataFile == '':
    print("Data file required.  Usage: cabd_dams_odi.py <datafile>")
    sys.exit()
...
update {workingTable} set owner = DAM_OWNERS;
update {workingTable} set ownership_type_code = 
    CASE
    WHEN DAM_OWNERS = 'Conservation Authority' THEN 1
    WHEN DAM_OWNERS = 'Municipal' THEN 3
    WHEN DAM_OWNERS = 'Private' THEN 4
    WHEN DAM_OWNERS = 'Provincial' THEN 5
    WHEN DAM_OWNERS = 'Ontario Power Generation' THEN 5
    ELSE NULL END;
...
update {workingTable} set data_source_id = OGF_ID;
update {workingTable} set data_source = 'ODI';
update {workingTable} set "comments" = GENERAL_CO;
