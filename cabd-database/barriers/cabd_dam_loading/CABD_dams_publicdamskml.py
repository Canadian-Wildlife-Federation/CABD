...
if dataFile == '':
    print("Data file required.  Usage: cabd_dams_publicdamskml.py <datafile>")
    sys.exit()
...
alter table {workingTable} rename height_m to height_m_orig;
...
update {workingTable} set owner = Dam_Owner;
...
update {workingTable} set operating_status_code = 
    CASE 
    WHEN Operation_ = 'Abandoned' THEN 1
    WHEN Operation_ = 'Active' THEN 2
    WHEN Operation_ = 'Application' THEN 5
    WHEN Operation_ = 'Breached (Failed)' THEN 5
    WHEN Operation_ = 'Deactivated' THEN 4
    WHEN Operation_ = 'Decommissioned' THEN 3
    WHEN Operation_ = 'Not Constructed' THEN 5
    WHEN Operation_ = 'Removed' THEN 3
    ELSE NULL end;
...
update {workingTable} set construction_year = cast(COMMISSION as numeric);
...
update {workingTable} set construction_type_code =
    CASE 
    WHEN Type = 'Combination' THEN 9
    WHEN Type = 'Concrete' THEN 9
    WHEN Type = 'Concrete–arch' THEN 1
    WHEN Type = 'Concrete–gravity' THEN 4
    WHEN Type = 'Concrete–slab/buttress' THEN 2
    WHEN Type = 'Earthfill' THEN 3
    WHEN Type = 'Embankment–homogenous' THEN 3
    WHEN Type = 'Embankment–unknown' THEN 3
    WHEN Type = 'Embankment–zoned' THEN 3
    WHEN Type = 'Log crib' THEN 8
    WHEN Type = 'Other' THEN 10
    WHEN Type = 'Rockfill' THEN 6
    WHEN Type = 'Steel' THEN 7
    WHEN Type = 'Unknown' THEN 9
    ELSE NULL end;
update {workingTable} set height_m = cast(height_m_orig as float4);
update {workingTable} set length_m = cast(Crest_Leng as float4);
update {workingTable} set size_class_code =
    CASE
    WHEN cast(height_m as float4) < 5 THEN 1
    WHEN cast(height_m as float4) >= 5 AND  cast(height_m as float4) < 15 THEN 2
    WHEN cast(height_m as float4) >= 15 THEN 3
    ELSE NULL END;
...
update {workingTable} set data_source_id = OBJECTID;
update {workingTable} set data_source = 'Public_Dams_KML';
