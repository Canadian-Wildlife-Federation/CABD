...
tempTableRaw = "dams_nhn_original"
...
if dataFile == '':
    print("Data file required.  Usage: LOAD_dams_nhn.py <datafile>")
    sys.exit()
...
UPDATE {tempTable} SET dam_name_en = name1;
...
UPDATE {tempTable} SET capture_date =
    ...
    ELSE NULL END;
UPDATE {tempTable} SET data_source = 'nhn_' || nid;
...
query = f"""
UPDATE
	load.dams_nhn AS nhn
SET
	cabd_id = duplicates.cabd_dam_id
FROM
	load.duplicates AS duplicates
WHERE
	nhn.data_source = duplicates.data_source
	OR nhn.data_source = duplicates.dups_nhn;
"""
