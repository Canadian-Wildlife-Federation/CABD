...
script = main.MappingScript("bcflnrord_wris_pubdams")
...
UPDATE dams.temp SET structure_type_new = 
    ...
    WHEN dam_type = 'Concrete–slab/buttress' THEN (SELECT code FROM dams.structure_type_codes WHERE name_en = 'Dam - Buttress')
    WHEN dam_type = 'Earthfill' THEN (SELECT code FROM dams.structure_type_codes WHERE name_en = 'Dam - Embankment')
    WHEN dam_type = 'Embankment–homogenous' THEN (SELECT code FROM dams.structure_type_codes WHERE name_en = 'Dam - Embankment')
    WHEN dam_type = 'Embankment–unknown' THEN (SELECT code FROM dams.structure_type_codes WHERE name_en = 'Dam - Embankment')
    WHEN dam_type = 'Embankment–zoned' THEN (SELECT code FROM dams.structure_type_codes WHERE name_en = 'Dam - Embankment')
    ...
    WHEN dam_type IN ('Combination', 'Concrete', 'Other') THEN (SELECT code FROM dams.structure_type_codes WHERE name_en = 'Dam - Other')
    ELSE NULL END;

UPDATE dams.temp SET construction_material_new =
    CASE
    WHEN dam_type ILIKE 'Concrete%' THEN (SELECT code FROM dams.construction_material_codes WHERE name_en = 'Concrete')
    ...
    WHEN dam_type = 'Combination' THEN (SELECT code FROM dams.construction_material_codes WHERE name_en = 'Other')
    ELSE NULL END;
