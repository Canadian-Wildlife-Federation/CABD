...
workingSchema = "load"
workingTableRaw = "waterfalls_fishwerks"
...
if dataFile == '':
    print("Data file required.  Usage: cabd_waterfall_fishwerks.py <datafile>")
    sys.exit()
...
alter table {workingTable} rename column latitude to latitude_orig;
alter table {workingTable} rename column longitude to longitude_orig;
...
update {workingTable} set waterbody_name_en = 'Lake ' || LAKE;
...
update {workingTable} set province_territory_code = 'on';
...
update {workingTable} set data_source_id = BARRIERID;
update {workingTable} set data_source = 'fishwerks';
...
updatequery = f"""
INSERT INTO waterfalls.waterfalls(cabd_id,fall_name_en,fall_name_fr,waterbody_name_en,waterbody_name_fr,watershed_group_code,nhn_workunit_id,province_territory_code,nearest_municipality,fall_height_m,capture_date,last_update,"comments",data_source_id,data_source,complete_level_code,original_point,snapped_point,passability_status_code,passability_status_note)
SELECT cabd_id,fall_name_en,fall_name_fr,waterbody_name_en,waterbody_name_fr,watershed_group_code,nhn_workunit_id,province_territory_code,nearest_municipality,fall_height_m,capture_date,last_update,"comments",data_source_id,data_source,complete_level_code,original_point,snapped_point,passability_status_code,passability_status_note
FROM {workingTable};
"""
