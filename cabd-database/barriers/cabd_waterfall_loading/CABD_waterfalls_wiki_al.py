...
workingSchema = "load"
workingTableRaw = "waterfalls_wiki"
...
if dataFile == '':
    print("Data file required.  Usage: cabd_waterfall_wiki.py <datafile>")
    sys.exit()
...
update {workingTable} set fall_name_en = Waterfall;
--update {workingTable} set fall_name_fr;
update {workingTable} set waterbody_name_en = Watercourse;
...
update {workingTable} set province_territory_code = 
    CASE
    WHEN Province is not null THEN lower(Province)
    ELSE NULL END;
--update {workingTable} set nearest_municipality;
update {workingTable} set fall_height_m = cast((regexp_split_to_array(waterfalls_wiki.drop, '\s'))[1] as float4);
--update {workingTable} set capture_date;
--update {workingTable} set last_update;
--update {workingTable} set "comments";
update {workingTable} set data_source_id = fid;
update {workingTable} set data_source = 'Wikipedia';
