...
script = main.DamLoadingScript("bc_hydro_wiki")
...
-- Finding CABD IDs...
UPDATE
	{script.workingTable} AS bc_hydro_wiki
SET
	cabd_id = duplicates.cabd_id
FROM
	{script.duplicatestable} AS duplicates
WHERE
	(bc_hydro_wiki.data_source_id = duplicates.data_source_id AND duplicates.data_source = 'bc_hydro_wiki') 
	OR bc_hydro_wiki.data_source_id = duplicates.dups_bc_hydro_wiki;    
"""
...
VALUES('{script.dsUuid}', 'bc_hydro_wiki', now(), null, null, 'Data update - ' || now());
