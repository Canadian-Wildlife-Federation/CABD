...
script = main.DamLoadingScript("wsa_sk")
...
-- Finding CABD IDs...
UPDATE
	{script.workingTable} AS wsa_sk
SET
	cabd_id = duplicates.cabd_id
FROM
	{script.duplicatestable} AS duplicates
WHERE
	(wsa_sk.data_source_id = duplicates.data_source_id AND duplicates.data_source = 'wsa_sk') 
	OR wsa_sk.data_source_id = duplicates.dups_wsa_sk;    
"""
...
VALUES('{script.dsUuid}', 'wsa_sk', now(), null, null, 'Data update - ' || now());
