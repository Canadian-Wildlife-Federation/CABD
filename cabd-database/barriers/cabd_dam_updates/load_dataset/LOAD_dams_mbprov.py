...
script = main.DamLoadingScript("mbprov")
...
-- Finding CABD IDs...
UPDATE
	{script.workingTable} AS mbprov
SET
	cabd_id = duplicates.cabd_id
FROM
	{script.duplicatestable} AS duplicates
WHERE
	(mbprov.data_source_id = duplicates.data_source_id AND duplicates.data_source = 'mbprov') 
	OR mbprov.data_source_id = duplicates.dups_mbprov;    
"""
...
VALUES('{script.dsUuid}', 'mbprov', now(), null, null, 'Data update - ' || now());
