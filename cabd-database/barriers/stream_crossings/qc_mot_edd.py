...
def loadData(conn, file):
    ...
    CREATE INDEX IF NOT EXISTS {targetTable}_geometry_midpoint_idx ON {schema}.{targetTable} USING gist (geometry_midpoint);
    """
...
def matchIds(conn):
    ...
    with match as (
        ...
        CROSS JOIN LATERAL (
            SELECT
            unique_id,
            ST_Distance(a.{mGeometry}, b.geometry_midpoint) as dist
            FROM {schema}.{targetTable} b
            ORDER BY a.{mGeometry} <-> b.geometry_midpoint
            LIMIT 1
        ) as nn
        WHERE nn.dist < {dist}
    ),
