...
def mapAttributes(conn):
    ...
    FROM {schema}.{targetTable} a WHERE cabd_id = a.crossing_id
    AND a.structuren ilike '%bridge%'
    AND (a.crossingty ilike '%nw' or a.crossingty IS NULL);

    UPDATE {schema}.modelled_crossings SET crossing_subtype = 'culvert'
    FROM {schema}.{targetTable} a WHERE cabd_id = a.crossing_id
    AND a.structuren ilike '%culvert%'
    AND (a.crossingty ilike '%nw' or a.crossingty IS NULL);
