...
script = main.MappingScript("glenmore_infrastructure")

mappingquery = f"""

--create new data source record
INSERT INTO cabd.data_source (id, name, version_date, version_number, source, comments)
VALUES('{script.dsUuid}', 'Glenmore Dam Infrastructure Improvements', now(), null, 'City of Calgary', 'Data update - ' || now());
...
SET    
   ...
   maintenance_last_ds = CASE WHEN {script.datasetname}.maintenance_last IS NOT NULL THEN {script.datasetname}.data_source ELSE cabdsource.maintenance_last_ds END,   
   condition_code_ds = CASE WHEN {script.datasetname}.condition_code IS NOT NULL THEN {script.datasetname}.data_source ELSE cabdsource.condition_code_ds END
FROM
...
SET
   ...
   maintenance_last = CASE WHEN {script.datasetname}.maintenance_last IS NOT NULL THEN {script.datasetname}.maintenance_last ELSE cabd.maintenance_last END,
   condition_code = CASE WHEN {script.datasetname}.condition_code IS NOT NULL THEN {script.datasetname}.condition_code ELSE cabd.condition_code END
FROM
   {script.workingTable} AS {script.datasetname}
WHERE
