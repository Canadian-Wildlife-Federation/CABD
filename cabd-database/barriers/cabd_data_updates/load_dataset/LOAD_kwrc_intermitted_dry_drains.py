...
script = main.LoadingScript("kwrc_intermitted_dry_drains")
...
UPDATE {script.nonTidalSites} SET constriction_code = (SELECT code FROM stream_crossings.constriction_codes WHERE name_en = 'severe') WHERE classification = 'too small';
UPDATE {script.nonTidalSites} SET crossing_comments = 
    CASE
...
CREATE TABLE {script.nonTidalStructures} AS (
    ...
    WHERE 
        ...
        AND source.classification != 'drain'
);
