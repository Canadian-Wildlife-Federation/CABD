...
select 
'with edges as (select c.id, a.nid from nhn' || short_name || '.eflowpath a join fpoutput.eflowpath b on a.id = b.id join chyf2.eflowpath c on b.internal_id  = c.id) update chyf2.eflowpath set nid = ed.nid from edges ed where ed.id = chyf2.eflowpath.id'
from chyf2.aoi;

--do the same for ecatchments
select id, short_name, 
'with wbs as (select c.id, a.nid from nhn' || short_name || '.ecatchment a join fpoutput.ecatchment b on a.id = b.id join chyf2.ecatchment c on b.internal_id  = c.id) update chyf2.ecatchment set nid = w.nid from wbs w where w.id = chyf2.ecatchment.id'
from chyf2.aoi

--to check that most nids are copies correctly:
...
select 
'with edges as (select b.id, a.nid from nhn' || name || '.eflowpath a join fpoutput.eflowpath b on a.id = b.id ) update fpoutput.eflowpath set nid = ed.nid from edges ed where ed.id = fpoutput.eflowpath.id'
from fpoutput.aoi where status != 'CHYF_DONE'

--same for ecatchments
select 
'with wbs as (select b.id, a.nid from nhn' || name || '.ecatchment a join fpoutput.ecatchment b on a.id = b.id ) update fpoutput.ecatchment set nid = w.nid from wbs w where w.id = fpoutput.ecatchment.id'
from fpoutput.aoi where status != 'CHYF_DONE'
