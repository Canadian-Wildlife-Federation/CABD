...
def mapAttributes(conn):
    ...
    UPDATE {schema}.modelled_crossings SET passability_status = 
        CASE
        WHEN a.fish_baffles = 'Y' THEN 'partial barrier'
        WHEN a.structure_type IN ('MAJOR BRIDGE', 'STANDARD BRIDGE', 'Undefined BRIDGE') AND a.structure_status != 'CANCELLED' THEN 'passable'
        ELSE NULL END
