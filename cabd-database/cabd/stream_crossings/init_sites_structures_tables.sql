...
BEGIN
    ...
    select 
        ...
        case when new_crossing_type is not null then new_crossing_type else null end as new_crossing_type,
    ...
    where status = 'REVIEWED' and (new_crossing_type is null or new_crossing_type not ilike 'dam');
   ...
   from (select cabd_id, count(*) from supdates group by cabd_id having count(*) > 1) foo;
    ...
    update stream_crossings.cwf_satellite_review set status = 'PROCESSED' where status = 'REVIEWED' and new_crossing_type not ilike 'dam';


    -- deal with dams cases
	
	--remove dams from site/structures tables
	delete from stream_crossings.structures_attribute_source where structure_id in (select structure_id from stream_crossings.structures a where a.site_id in (select cabd_id from stream_crossings.cwf_satellite_review where status = 'REVIEWED' and new_crossing_type ilike 'dam'));
    delete from stream_crossings.sites_attribute_source where cabd_id in (select cabd_id from stream_crossings.cwf_satellite_review where status = 'REVIEWED' and new_crossing_type ilike 'dam');
    delete from stream_crossings.structures where site_id in (select cabd_id from stream_crossings.cwf_satellite_review where status = 'REVIEWED' and new_crossing_type ilike 'dam');
    delete from stream_crossings.sites where cabd_id in (select cabd_id from stream_crossings.cwf_satellite_review where status = 'REVIEWED' and new_crossing_type ilike 'dam');
    ...
    from stream_crossings.cwf_satellite_review 
    where status = 'REVIEWED' and new_crossing_type ilike 'dam' and create_dam ;
    ...
    update stream_crossings.cwf_satellite_review set status = 'PROCESSED' where status = 'REVIEWED' and new_crossing_type ilike 'dam';
