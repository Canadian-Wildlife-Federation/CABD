...
CREATE TABLE dams.dams_medium_large_attribute_source (
	...
	nearest_municipality_dsfid varchar,
	...
	capture_date_dsfid varchar,
	last_update_ds uuid REFERENCES cabd.data_source(id),
	last_update_dsfid varchar,
...
capture_date_ds, capture_date_dsfid, last_update_ds, last_update_dsfid, comments_ds, comments_dsfid, 
...
case when a.nearest_municipality is not null then b.id else null end,
case when a.nearest_municipality is not null then a.data_source_id else null end,
...
case when a.capture_date is not null then b.id else null end,
case when a.capture_date is not null then a.data_source_id else null end,
case when a.last_update is not null then b.id else null end,
case when a.last_update is not null then a.data_source_id else null end,
...
waterbody_name_en_dsfid, waterbody_name_fr_ds, waterbody_name_fr_dsfid, nearest_municipality_ds, nearest_municipality_dsfid, fall_height_m_ds,
fall_height_m_dsfid, capture_date_ds, capture_date_dsfid, last_update_ds, last_update_dsfid, comments_ds,
comments_dsfid, complete_level_code_ds, complete_level_code_dsfid, passability_status_code_ds,
passability_status_code_dsfid, original_point_ds, original_point_dsfid)
...
nearest_municipality_ds, 
nearest_municipality_dsfid, fishpass_type_code_ds, fishpass_type_code_dsfid, 
